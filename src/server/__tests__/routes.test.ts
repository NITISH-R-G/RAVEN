...
describe('POST /api/analyze', () => {
  ...
  it('should not inject invalid client fingerprint IDs', async () => {
    const maliciousFingerprint = 'fp-test<script>alert(1)</script>';

    const response = await request(app)
      .post('/api/analyze')
      .send({
        engineMode: 'local',
        clientFingerprintId: maliciousFingerprint,
        documents: [{ id: '1', name: 'test.pdf', type: 'ID_PROOF', content: 'test fp-88a29b4e' }],
      });
    ...
    const calls = analyzeDocumentsDynamically.mock.calls;
    const passedDocs = calls[calls.length - 1][0];

    expect(passedDocs[0].content).not.toContain(maliciousFingerprint);
    expect(passedDocs[0].content).toContain('fp-88a29b4e');
  });
});
