...
interface AnalyzeRequestBody {
  useManagedAgent?: string | boolean;
...
router.post("/api/analyze", analyzeLimiter, upload.array("files"), async (req: Request<unknown, unknown, AnalyzeRequestBody>, res: Response) => {
...
Analyze the documents below. You MUST respond in valid JSON format. Follow the strict schema exactly.`;
    ...
    const errObj: ApiError = isApiError(error) ? error : {};
    ...
    let cleanSummary = `Fallback active (Engine exception: ${errMsg}). ${fallback.summary}`;
    if (isQuotaExceeded) {
