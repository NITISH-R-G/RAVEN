...
router.post('/api/analyze', analyzeLimiter, upload.array('files'), async (req, res) => {
  ...
  if (files && files.length > 0) {
    const filePromises = files.map(async (file) => {
      ...
      return {
        id: `uploaded-${Date.now()}-${crypto.randomUUID()}`,
    ...
    const parsedDocuments = await Promise.all(filePromises);
...
Analyze the documents below. You MUST respond in valid JSON format. Follow the strict schema exactly.`;
    ...
    let cleanSummary = `Fallback active (Engine exception: ${errObj.message || 'Unknown error'}). ${fallback.summary}`;
    if (isQuotaExceeded) {
