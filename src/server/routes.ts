...
router.post('/api/analyze', analyzeLimiter, upload.array('files'), async (req, res) => {
  ...
  // Validate fingerprint ID to prevent text injection vulnerabilities
  const isValidFingerprint =
    typeof clientFingerprintId === 'string' && /^[a-zA-Z0-9_-]+$/.test(clientFingerprintId);

  if (isValidFingerprint && documents.length > 0) {
