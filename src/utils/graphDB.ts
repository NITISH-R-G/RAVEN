  ...
  public findFraudRings(): TraversalReport[] {
    ...
    for (let d = 0; d < devices.length; d++) {
      ...
      for (let i = 0; i < incomingEdges.length; i++) {
        const node = this.vertices.get(incomingEdges[i].target);
      ...
      if (connectedPersons.length > 1) {
        ...
        for (let p = 0; p < connectedPersons.length; p++) {
          const person = connectedPersons[p];
          steps.push({
            nodeId: person.id,
            label: person.label,
            type: person.type,
    ...
    for (let p = 0; p < properties.length; p++) {
      ...
      if (!prop.id.includes('flat402')) continue;
      ...
      for (let i = 0; i < adjacentEdges.length; i++) {
        const node = this.vertices.get(adjacentEdges[i].target);
        if (node && node.type === 'person' && node.status === 'flagged') {
          linkedApplicants.push(node);
        }
      }

      if (linkedApplicants.length > 1) {
        ...
        for (let a = 0; a < linkedApplicants.length; a++) {
          const app = linkedApplicants[a];
          steps.push({
            nodeId: app.id,
            label: app.label,
            type: app.type,
