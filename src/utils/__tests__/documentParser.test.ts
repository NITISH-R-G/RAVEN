...
describe('parseDocument', () => {
  ...
  describe('SALARY_SLIP Document Type', () => {
    ...
    it('detects discrepancy in employer', () => {
      const doc: DocumentItem = {
        id: 'doc-2',
        content: 'Employer: MegaCorp Discrepancy',
        type: 'SALARY_SLIP',
        name: 'salary.pdf',
      };
      const result = parseDocument(doc, 'fingpt-llama');
  ...
  describe('PROPERTY_VALUATION Document Type', () => {
    ...
    it('flags double-mortgaged status', () => {
      const doc: DocumentItem = {
        id: 'doc-3',
        content: 'Valuation Amount: 5000000\nNote: Double-mortgaged property',
        type: 'PROPERTY_VALUATION',
        name: 'prop.pdf',
      };
      const result = parseDocument(doc, 'anthropic-finance');
  ...
  describe('General/Fallback Document Type', () => {
    it('extracts device and IP details', () => {
      const text = ['Session IP: 192.168.1.1', 'Device ID: device-123'].join('\n');
      ...
      const deviceEntity = result.entities.find(
        (e) => e.field === 'Extracted Target Device Engine Signature',
      );
    ...
    it('handles anomaly in device ID', () => {
      const doc: DocumentItem = {
        id: 'doc-4',
        content: 'Device ID: unknown anomaly',
        type: 'OTHER',
        name: 'log.txt',
      };
      const result = parseDocument(doc, 'fingpt-llama');
    ...
    it('handles mismatch in IP', () => {
      const doc: DocumentItem = {
        id: 'doc-4',
        content: 'Session IP: 10.0.0.1 mismatch',
        type: 'OTHER',
        name: 'log.txt',
      };
      const result = parseDocument(doc, 'fingpt-llama');
