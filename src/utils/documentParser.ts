...
export const parseDocument = (
  ...
  if (type === 'ITR') {
    ...
    const incomeMatch = findValue([
      'gross total income',
      'gross revenue declared',
      'revenue submitted',
      'taxable',
    ]);
    const employerMatch = findValue([
      'employer name',
      'employment status',
      'business activity',
      'company',
    ...
    if (incomeMatch) {
      ...
      tempEntities.push({
        ...
        status:
          valNum > 2000000 && text.toLowerCase().includes('rajesh') ? 'discrepant' : 'verified',
      });
    }
    ...
    const salaryMatch = findValue([
      'gross salary',
      'gross credit details',
      'direct credits paid',
      'net disbursed',
    ]);
    ...
    if (salaryMatch) {
      tempEntities.push({
        ...
        status:
          text.toLowerCase().includes('discrepancy') || text.toLowerCase().includes('photoshop')
            ? 'discrepant'
            : 'verified',
      });
    }
